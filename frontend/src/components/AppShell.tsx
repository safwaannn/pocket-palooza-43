...
function useUnreadAlerts() {
  return useQuery({
    ...
    queryFn: async (): Promise<number> => {
      try {
        const res = await api<{ count: number }>("/budget-alerts/unread-count");
        return res?.count ?? 0;
      } catch {
        return 0;
      }
        ...
        .map((item) => {
          ...
          return (
            ...
            >
              ...
              <Icon
                aria-hidden="true"
                className={`h-4 w-4 transition-colors ${active ? "text-primary-glow" : ""}`}
              />
...
export function AppShell({ title, children }: { title: string; children: ReactNode }) {
  ...
  const Brand = (
    <div className="flex items-center gap-2.5 px-4 py-5">
      ...
      <div className="flex flex-col leading-none">
        ...
        <span className="mt-0.5 text-[10px] font-medium uppercase tracking-[0.18em] text-white/40">
          Finance Tracker
        </span>
      </div>
    </div>
  ...
  return (
    <div className="flex min-h-dvh">
      ...
      <aside
        aria-label="Sidebar"
        className="hidden md:flex w-64 flex-col bg-gradient-sidebar text-white"
      >
        {Brand}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border/60 bg-card/70 px-4 backdrop-blur-xl md:px-8">
          <Sheet>
            ...
            <SheetContent
              ...
              className="w-64 bg-gradient-sidebar p-0 text-white border-r-0"
            >
              {Brand}
        ...
        <main
          id="main-content"
          tabIndex={-1}
          className="flex-1 overflow-x-hidden p-4 md:p-8 focus:outline-none"
        >
          {children}
        </main>
      </div>
