...
import { parseTransactionsCSV, type ParsedTransaction } from "@/lib/csv-import";
...
export function ImportCSVDialog({ trigger }: { trigger: React.ReactNode }) {
  ...
  const importMut = useMutation({
    mutationFn: async () => {
      ...
      // Resolve a category id for each (type, name), creating any that are
      // missing. The backend has no bulk-import endpoint, so we create
      // categories then post transactions individually — mirroring the old
      ...
      const ensureCategory = async (name: string, type: "income" | "expense") => {
        ...
        const created = await api<unknown>("/categories", {
          method: "POST",
          body: { name, type },
        });
        const mapped = mapCategory(created as never);
        catIndex.set(key, mapped.id);
        return mapped.id;
      };

      let count = 0;
      for (const row of preview.rows) {
        const categoryId = await ensureCategory(row.category, row.type);
    ...
    >
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV with columns: <code>date, type, category, amount, note</code>. Dates must
            be YYYY-MM-DD, type must be <code>income</code> or <code>expense</code>. Missing
            categories will be created for you.
          </DialogDescription>
        </DialogHeader>
