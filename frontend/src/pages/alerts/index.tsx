...
export function AlertsPage() {
    ...
    queryFn: async (): Promise<AlertRow[]> => {
      const data = await api<
        Array<{
          id?: string;
          _id?: string;
          category?: { name?: string; id?: string; _id?: string } | string | null;
          month_year: string;
          threshold: number;
          acknowledged: boolean;
          createdAt?: string;
        }>
      >("/budget-alerts?sort=-createdAt&limit=100");
      return (data ?? []).map((a) => {
        const cat = a.category;
        const categoryObj = cat && typeof cat === "object" ? cat : null;
        return {
          id: a.id ?? a._id ?? "",
          category_id:
            (categoryObj?.id ?? categoryObj?._id ?? (typeof cat === "string" ? cat : "")) ?? "",
          month_year: a.month_year,
          threshold: a.threshold,
          acknowledged: a.acknowledged,
          created_at: a.createdAt ?? "",
          categories: categoryObj?.name ? { name: categoryObj.name } : null,
        };
      });
    },
  });
  ...
  const ackOne = useMutation({
    mutationFn: async (id: string) => {
      ...
      await api(`/budget-alerts/${id}/acknowledge`, { method: "PATCH" });
    },
  ...
  const ackAll = useMutation({
    mutationFn: async () => {
      ...
      await api("/budget-alerts/acknowledge-all", { method: "POST" });
    },
    onSuccess: () => {
