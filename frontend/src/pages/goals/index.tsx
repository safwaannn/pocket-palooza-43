...
export function GoalsPage() {
  ...
  useEffect(() => {
    // Goals are stored in localStorage keyed by a static user id until the new
    // backend auth is wired up. Replace with a fetch('/api/v1/users/me') call.
    const fallbackId = "local-user";
    setUserId(fallbackId);
    setGoals(loadGoals(fallbackId));
    try {
