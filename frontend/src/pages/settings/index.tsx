...
export function SettingsPage() {
  ...
  const updateName = useMutation({
    mutationFn: async () => {
      ...
      await api("/users/updateMe", { method: "PATCH", body: { name: name.trim() || null } });
    },
    onSuccess: () => {
  ...
  const updateCurrency = useMutation({
    mutationFn: async (next: string) => {
      if (!profile) throw new Error("No profile");
      await api("/users/updateMe", { method: "PATCH", body: { currency: next } });
    },
    onSuccess: () => {
    ...
    <AppShell title="Settings">
      <div className="mx-auto max-w-2xl space-y-6">
        <div className="pb-2">
          ...
          <h2 className="font-display text-3xl font-semibold tracking-tight text-gradient-brand">
            Settings
          </h2>
